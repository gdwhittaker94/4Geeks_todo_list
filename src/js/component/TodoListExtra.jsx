...
const TodoList = () => {
    ...
    const [prevUserValue, setPrevUserValue] = useState("");       // Prev User 
    const url = "https://playground.4geeks.com/apis/fake/todos/user/" //My User URL
   ...
   const getData = () => {                               // Fetching Data using Fetch API 
    ...
    const createUser = () => {      
        console.log({ url })                      // Function to create user in API if not already present
    ...
    const createNewUser = () => {  
        console.log(userValue)                       // Function to create user in API if not already present
        fetch(`${url}${userValue}`, {                                      // Fetch endpoint + Fetch init object to adjust settings of fetch
            method: "POST",                               // POST = Create
            headers: {                                    // Headers = meta-data/additional info about our request 
                "Content-Type": "application/json"        // Indicates the type of data being sent in the request body to the server.    
            },
    ...
    return (
        <>
            ...
            <input
            type="text" 
            placeholder='Recover Previous User...'    
            value={prevUserValue}                                                     // Value of input = value in "userValue" variable = CONTROLLED INPUT = value controlled by React's state 
            onChange={(event) => setPrevUserValue(event.target.value)}                // "userValue" variable synced up with input field's value 
            onKeyDown={(event) => {if(event.key === "Enter"){getData()}}} 
            />
            <input 
                type="text" 
